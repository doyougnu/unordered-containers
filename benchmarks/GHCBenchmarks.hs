...
delete xs m0 = foldl' (flip HM.delete) m0 xs
...
alterDelete :: (Eq k, Hashable k) => [k] -> HM.HashMap k Int
             -> HM.HashMap k Int
alterDelete xs m0 =
  foldl' (flip (HM.alter (const Nothing))) m0 xs
...
deleteM xs m0 = foldl' (flip M.delete) m0 xs
...
deleteIHM xs m0 = foldl' (flip IHM.delete) m0 xs
...
deleteIM xs m0 = foldl' (flip IM.delete) m0 xs

------------------------------------------------------------------------
-- * GHC benchmarks

alterInsertLookupIM :: IM.IntMap Int -> [(Int, Int)] -> IM.IntMap Int
alterInsertLookupIM = foldl' go
  where
    go m (k,v) | v == 0    = fromMaybe 0 (IM.lookup k m) `seq` m
               | otherwise = IM.insert k v m

alterInsertLookupHM :: HM.HashMap Int Int -> [(Int, Int)] -> HM.HashMap Int Int
alterInsertLookupHM = foldl' go
  where
    go m (k,v) | v == 0    = fromMaybe 0 (HM.lookup k m) `seq` m
